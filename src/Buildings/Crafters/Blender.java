...
public class Blender extends Building {
    ...
    public Blender(Point position, boolean recipeSet, Recipe recipe, double overclock) {
        this.position = position;
    ...
    private void initializeRecipes() {
        possibleRecipes = Recipes.getRecipeByBuilding(BuildingType.BLENDER);
    }

    private void getImages() {
        try {
            image = ImageIO.read(new File("resources/images/buildings/blender.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void clicked(GraphicsPanel gp, int mx, int my) {
        ...
        if (mx > menuTopLeft.x + 10 && mx < menuTopLeft.x + 95 && my > menuTopLeft.y + 260 && my < menuTopLeft.y + 290) {
            gp.closeBuildingMenu();
            gp.addBuilding(new Blender(position, recipeSet, recipe, overclock));
        }
    ...
    public void drawMenu(Graphics2D g2d) {
        ...
        g2d.drawString("Blender", menuTopLeft.x + 10, menuTopLeft.y + 25);
