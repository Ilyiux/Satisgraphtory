...
public class Conveyor extends Connector {
    ...
    private void updateRate() {
        ...
        if (tier == 1) maxRate = 60;
        if (tier == 2) maxRate = 120;
        if (tier == 3) maxRate = 270;
        if (tier == 4) maxRate = 480;
        if (tier == 5) maxRate = 780;
    ...
    public void drawMenu(Graphics2D g2d) {
        ...
        g2d.setColor(tier == 5 ? disabledColor : enabledColor);
